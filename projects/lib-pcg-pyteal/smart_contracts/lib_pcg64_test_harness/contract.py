from typing import Literal

import beaker
import pyteal as pt

from lib_pcg.pcg64 import (
    pcg64_init,
    pcg64_random,
)
from smart_contracts.consts import (
    MAX_UINT64_IN_STACK_ARRAY,
)

app = beaker.Application("lib_pcg64_test_harness_pyteal")


@app.external
def get_pcg64_sequence_arc4_uint64_return(
    seed: pt.abi.StaticBytes[Literal[16]],
    lower_bound: pt.abi.Uint64,
    upper_bound: pt.abi.Uint64,
    length: pt.abi.Uint16,
    *,
    output: pt.abi.DynamicArray[pt.abi.Uint64]
) -> pt.Expr:
    rng_handle_state1 = pt.ScratchVar(pt.TealType.uint64)
    rng_handle_state2 = pt.ScratchVar(pt.TealType.uint64)

    return pt.Seq(
        pcg64_init(
            rng_handle_state1.index(),
            rng_handle_state2.index(),
            seed.get(),
        ),
        output.decode(
            pcg64_random(
                rng_handle_state1.index(),
                rng_handle_state2.index(),
                lower_bound.get(),
                upper_bound.get(),
                length.get(),
            )
        ),
    )


@app.external
def runtime_asserts_pcg64_stack_array() -> pt.Expr:
    rng_handle_state1 = pt.ScratchVar(pt.TealType.uint64)
    rng_handle_state2 = pt.ScratchVar(pt.TealType.uint64)

    return pt.Seq(
        pcg64_init(
            rng_handle_state1.index(),
            rng_handle_state2.index(),
            pt.BytesZero(pt.Int(16)),
        ),
        pt.Pop(
            pcg64_random(
                rng_handle_state1.index(),
                rng_handle_state2.index(),
                pt.Int(0),
                pt.Int(0),
                pt.Int(MAX_UINT64_IN_STACK_ARRAY),
            )
        ),
    )


@app.external
def runtime_failure_stack_byteslice_overflow() -> pt.Expr:
    rng_handle_state1 = pt.ScratchVar(pt.TealType.uint64)
    rng_handle_state2 = pt.ScratchVar(pt.TealType.uint64)

    return pt.Seq(
        pcg64_init(
            rng_handle_state1.index(),
            rng_handle_state2.index(),
            pt.BytesZero(pt.Int(16)),
        ),
        pt.Pop(
            pcg64_random(
                rng_handle_state1.index(),
                rng_handle_state2.index(),
                pt.Int(0),
                pt.Int(0),
                pt.Int(MAX_UINT64_IN_STACK_ARRAY + 1),
            )
        ),
    )
