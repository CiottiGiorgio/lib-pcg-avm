...
@pytest.fixture(scope="session")
def max_unbounded_opup_calls(request: pytest.FixtureRequest) -> int:
    match request.config.getoption("language"):
        case "algopy":
            return 12
        case "algots":
...
@pytest.fixture(scope="session")
def max_bounded_opup_calls(request: pytest.FixtureRequest) -> int:
    match request.config.getoption("language"):
        case "algopy":
            return 14
        case "algots":
...
def test_unbounded_maximal_cost(
    ...
    result = lib_pcg32_harness.get_pcg32_sequence_arc4_uint32_return(
        RNG_SEED, 0, 0, expected_maximal_sequence_length
    )
...
def test_bounded_maximal_cost(
    ...
    result = lib_pcg32_harness.get_pcg32_sequence_arc4_uint32_return(
        RNG_SEED, 1, 2**32 - 1, expected_maximal_sequence_length
    )
